...
* three different sequence number generator algorithms included with their
...
// Fibonacci Series using Space Optimized Method runs Θ(n) and space utiization of O(1).
int fib_1(int n) {
    int a = 0, b = 1, c, i;
    if (n == 0) {
        return a;
...
// Fibonacci Series using Dynamic Programming runs O(n) but also has a space utilization of O(n) so NOT USED.
int fib_2(int n) {
    int i;
