...
int main() {
    ...
    vector<int> solution;                                         // Vector of max distances for each vertex as source.
...
void Dijkstra(int graph[V][V], int src, vector<int> &solution) {
    ...
    // Distance source vertex from itself is 0.
    dist[src] = 0;
	...
	for (int count = 0; count < V - 1; count++) {
		...
		for (int v = 0; v < V; v++) {
			/* Update dist[v] only if is not in sptSet, there is an edge from u to v, and total weight
